...
class AudioStruct(object):
  def __init__(self): #file_path,config):
    # Constants
    self.save_spec_img = False
    self.save_spec_img_dir = 'images'
    self.song_samples = 660000
    self.file_path = 'Data/genres_original/'
    self.n_fft = 2048
    self.hop_length = 512
  ...
  def getdata(self):
    ...
    return np.array(song_data), tensorflow.keras.utils.to_categorical(genre_data, len(self.genres))
